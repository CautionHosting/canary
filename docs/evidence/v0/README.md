...
- `canaryctl save-canary-keys` with independently verified Canary PCRs.
- Download plus offline `canaryctl verify-statement` and `verify-evidence` for each
  target. The saved public keys must be the exact document authenticated by
  `save-canary-keys`.
...
| Metadata/config/key mismatch | `save-canary-keys` rejects it |  |  |
...
| [8](../../canary-v0-spec.md#17-acceptance-criteria) | Independent `save-canary-keys` result |
