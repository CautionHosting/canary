//! Pure verification of Caddy's attested TLS metadata against one observed
...
pub const CADDY_MODE: &str = "caddy";
...
/// Build a signed diagnostic result from authenticated Caddy metadata and the
/// leaf certificate observed on the exact attestation response connection.
pub fn evaluate_caddy_binding(
    user_data: Option<&[u8]>,
    peer_certificate_der: Option<&[u8]>,
) -> Option<TlsBindingResult> {
    let metadata: CaddyUserData = serde_json::from_slice(user_data?).ok()?;
...
mod tests {
    ...
    fn matching_metadata_binds_the_observed_leaf() {
        ...
        let result = evaluate_caddy_binding(
            Some(&user_data(CADDY_MODE, "app.example.com", &certfp)),
            Some(certificate),
        )
        .unwrap();
    ...
    fn valid_but_unequal_metadata_is_retained_for_signed_diagnostics() {
        ...
        let wrong_domain = evaluate_caddy_binding(
            Some(&user_data(CADDY_MODE, "other.example.com", &observed)),
            Some(certificate),
        )
        .unwrap();
        ...
        let wrong_fingerprint = evaluate_caddy_binding(
            Some(&user_data(CADDY_MODE, "app.example.com", &"a".repeat(64))),
            Some(certificate),
        )
        .unwrap();
    ...
    fn missing_or_noncanonical_inputs_fail_closed() {
        assert_eq!(evaluate_caddy_binding(None, Some(b"certificate")), None);
        assert_eq!(
        ...
        for malformed in [
            br#"{}"#.as_slice(),
            br#"{"tls":{"mode":"caddy","domain":"App.example.com","certfp":"aa"}}"#,
            br#"{"tls":{"mode":"caddy\n","domain":"app.example.com","certfp":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}}"#,
            br#"{"tls":{"mode":"caddy","domain":"app.example.com","certfp":"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"}}"#,
            br#"{"tls":{"mode":"caddy","domain":"app.example.com","certfp":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","extra":true}}"#,
        ] {
            assert!(evaluate_caddy_binding(Some(malformed), Some(b"certificate")).is_none());
        }
    }
    ...
    fn wrong_mode_is_well_formed_but_does_not_match() {
        let result = evaluate_caddy_binding(
            Some(&user_data("steve", "app.example.com", &"a".repeat(64))),
            Some(b"certificate"),
        )
        .unwrap();
        assert!(!result.matches("app.example.com"));
